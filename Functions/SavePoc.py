...
def generate_filename(content, tool_name):
    # 在内容中查找形如 "id: xxx" 的字符串
    match = re.search(r'id:\s*(\S+)', content)
    if match:
        # 提取出id并组合成文件名
        return match.group(1) + "_" + tool_name + ".yaml"
    else:
        return "default_" + tool_name + ".yaml"  # 如果没有找到id，则使用默认文件名
...
def save_poc(window):
    ...
    if window.ui.radioButton.isChecked():
        ...
        selected_tool = "X-ray"
    # 生成默认文件名
    default_filename = generate_filename(content, selected_tool)
    # 打开文件对话框并填充默认文件夹和文件名
